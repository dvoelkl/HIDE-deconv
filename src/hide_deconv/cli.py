...
@assert_trained
def cli_analyze_kmean(hidedeconv_path: Path) -> None:
    """
    Performs k-means clustering and saves corresponding cluster assignment and plots.
    """
    from .cli_commands import create_kmean_plot

    create_kmean_plot(hidedeconv_path)
...
@assert_trained
def cli_analyze_scatter(hidedeconv_path: Path) -> None:
    """
    Plot genes relevant for deconvolution as heatmap.
    """
    from .cli_commands import create_celltype_scatter_plot

    create_celltype_scatter_plot(hidedeconv_path)
